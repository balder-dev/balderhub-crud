...
class ScenarioSingleUpdate(balder.Scenario):
    ...
    class DeviceUnderTest(balder.Device):
        ...
        single_reader = SingleDataReaderFeature()
        updater = SingleDataUpdaterFeature()
        example = ExampleFieldValueProviderFeature()

    @balder.parametrize_by_feature('cur_fillable_field', (DeviceUnderTest, 'updater', 'resolved_fillable_fields'))
    @balder.parametrize_by_feature('valid_field_value', (DeviceUnderTest, 'example', 'get_valid_new_value_for_field'),
                                   parameter={
    ...
    def test_field_update_valid(self, cur_fillable_field, valid_field_value):
        ...
        assert pot_data.compare(item_to_update_in_before, allow_non_definable=True)  # TODO should we allow Non-Definable here?
        ...
        assert new_expected_data.compare(item_to_update_in_after, allow_non_definable=True)  # TODO should we allow Non-Definable here?

        assert all_items_before_copy.compare(all_items_after_copy)


    @balder.parametrize_by_feature('cur_fillable_field', (DeviceUnderTest, 'updater', 'resolved_fillable_fields'))
    @balder.parametrize_by_feature('invalid_field_value', (DeviceUnderTest, 'example', 'get_invalid_new_value_for_field'),
                                   parameter={
                                       'data_item': parametrization.FeatureAccessSelector(PointOfTruth, 'data', 'data_item'),
                                       'field': parametrization.Parameter('cur_fillable_field')
                                   })
    def test_field_update_invalid(self, cur_fillable_field, invalid_field_value: ExampleFieldValueProviderFeature.NamedExample):
        pot_data = self.PointOfTruth.data.data_item
        ...
        assert all_items_before.compare(all_items_after, allow_non_definable=True)  # TODO should we allow Non-Definable here?
