...
class SetupCategory(balder.Setup):
    ...
    class Dut(balder.Device):
        ...
        initial_data = balderhub.data.lib.setup_features.factories.AutoInitialDataConfigFactory.get_for(BookCategoryDataItem)()

    @balder.connect(Dut, over_connection=balder.Connection)
    class Client(balder.Device):
        accessible_data = balderhub.data.lib.setup_features.factories.AutoAccessibleInitialDataConfigFactory.get_for(BookCategoryDataItem)(Master='Dut')
        ...
        read_example = balderhub.crud.lib.setup_features.factories.AutoSingleReadExampleFactory.get_for(BookCategoryDataItem, return_style='first')()
        update_example = category.ExampleSingleUpdateCategoryFieldProvider()
        # TODO delete_example = balderhub.crud.lib.setup_features.factories.AutoSingleDeleteExampleFactory.get_for(BookCategoryDataItem, return_style='first')()
