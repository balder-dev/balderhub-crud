...
class ExampleCreateBookProvider(balderhub.crud.lib.scenario_features.SingleCreateExampleProvider):

    class Dut(balder.VDevice):
        sim = DutSimulatorFeature()
        env = DataEnvironmentFeature()

    def get_valid_examples(self) -> list[balderhub.crud.lib.scenario_features.SingleCreateExampleProvider.NamedExample]:
    ...
    def get_invalid_examples(self) -> list[balderhub.crud.lib.scenario_features.SingleCreateExampleProvider.NamedExample]:
        author = self.Dut.env.get(data_items.AuthorDataItem, 1)
        category = self.Dut.env.get(data_items.BookCategoryDataItem, 1)
        return [
            self.NamedExample(
                name='Book with empty title',
                data_item=data_items.BookDataItem(id=NOT_DEFINABLE, title='', author=author, category=category),
                expected_response_messages=ResponseMessageList(
            ...
            self.NamedExample(
                ...
                data_item=data_items.BookDataItem(
            ...
            self.NamedExample(
                ...
                data_item=data_items.BookDataItem(id=NOT_DEFINABLE, title='The Hobbit', author=author,
