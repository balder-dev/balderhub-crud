...
class SingleAuthorUpdator(SingleDataUpdaterFeature):

    class Dut(balder.VDevice):
        sim = DutSimulatorFeature()
        single_data = factories.AutoSingleDataConfigSetupFactory.get_for(AuthorDataItem)()
    ...
    def load(self):
        self._data = copy.deepcopy(self.Dut.sim.dut_simulator.get_all_authors().get_by_identifier(self.Dut.single_data.data_item.id))
    ...
    def save(self):
        ...
        try:
            self.Dut.sim.dut_simulator.update_in_author(with_id=self._data.id, data_to_update=data_to_update)
            self._data = None
